...
The boundary operators of a complex are integer matrices the encode the adjacency relations between $k$- and $k - 1$-dimensional cells.
...
A regular $n$-dimensional CW complex consists of an $n - 1$-dimensional complex $X^{n - 1}$ together with a collection $\{\phi_\alpha^n\}$ of homeomorphisms, called \emph{attaching maps}, from the unit sphere in $S^{n - 1}$ into $X^{n - 1}$.
